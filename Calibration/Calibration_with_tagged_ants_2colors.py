...
norm_mat = None  # np.loadtxt(open(r"Z:\Lior&Einav\Calibration\videos\illumination 1_5_19\NormMatrix_rhod.csv", "rb"),
# delimiter=",", dtype='float32')

feedings = [2, 3, 4, 5]
for f in feedings:
    path = r'Y:\Lior&Einav\Calibration\Rhod_Atto\Ant Calibration 29_6_20\feed' + str(f)
    outpath = path + sep + 'blob analysis'
    ex1 = Experiment.Experiment2Colors(path, tags_vid_prefix='feed')
    ...
    ants_info_all_frames = ants_info.parse_tag_data(tagfile_format='bugtag')
    ...
    while available_frame_to_read_tags and available_frame_to_read_fluo:
        ...
        intensity_line = dict()
        area_line = dict()
        intensity_line['acquisition'] = acquisition
        area_line['acquisition'] = acquisition
