import cv2
from raw_data_processing import Experiment, Frame
from os import sep as sep

expName = '8_18_6_19'
path = r'Z:\Lior&Einav\Experiments\Experiment_'+expName+'\With food'
outpath = path + sep + 'blob analysis'
ex1 = Experiment.Experiment2Colors(path, get_timestamps='False')
trim_coordinates, frame_sizes, transformation_matrix = ex1.get_transformation_parameters()
bg_masks = ex1.define_background_to_remove_from_fluorescence_frame(trim_coordinates,32)
food_source_masks = ex1.get_food_source_masks(trim_coordinates, frame_sizes, transformation_matrix)

# norm_mats = dict()
# norm_mats['BLGF'] = np.loadtxt(open(r"Z:\Lior&Einav\Calibration\videos\illumination 1_5_19\NormMatrix_atto.csv", "rb"), delimiter=",",dtype='float32')
# norm_mats['GLRF'] = np.loadtxt(open(r"Z:\Lior&Einav\Calibration\videos\illumination 1_5_19\NormMatrix_rhod.csv", "rb"), delimiter=",",dtype='float32')
...
while available_frame_to_read:
    ...
    if current_frame not in ex1.missing_frames:
        tag_frame, fluo_frame, output_frame, available_frame_to_read, acquisition, th = \
            ex1.create_combined_frame(trim_coordinates, frame_sizes, transformation_matrix, bg_masks)
    ...
    #
    #             output_line.update(ant.to_dict(['x','y','angle','error','crop_intensity','copr_area','original_x','original_y']))
    #
